...
func (ov *OrderedValue) ValueForKey(key string) (*OrderedValue, bool) {
	...
	if obj, isObj := ov.Value.(OrderedObject); !isObj {
		return nil, false
	} else {
		for _, orderedPair := range obj {
			if orderedPair.Key == key {
				return orderedPair.Value, true
			}
		}
	}
	return nil, false
...
func (ov *OrderedValue) ArrayValueForKey(key string) (OrderedArray, bool) {
	if v, found := ov.ValueForKey(key); found {
		if v.Type == ARRAY {
			if s, ok := v.Value.(OrderedArray); ok {
				return s, true
			}
		}
	}
	return OrderedArray{}, false
}

func (ov *OrderedValue) ObjectValueForKey(key string) (OrderedObject, bool) {
	if v, found := ov.ValueForKey(key); found {
		if v.Type == OBJECT {
			if s, ok := v.Value.(OrderedObject); ok {
				return s, true
			}
		}
	}
...
func traversToValue[T any](o *OrderedValue, errorValue T, fn func(*OrderedValue) (T, error)) (T, error) {
	...
	case BOOL, INTEGER, STRING, NUMBER:
		return fn(o)
	default:
...
func (o *OrderedValue) GetValue() (any, ValueType, error) {
	...
	if b, err := o.NumberValue(); err == nil {
		...
		return "", NULL, fmt.Errorf("couldn't detect value type")
	}
}
